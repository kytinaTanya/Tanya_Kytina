...
class RatedItemsRepositoryImpl @Inject constructor(val service: TmdbService) :
    RatedItemsRepository {

    override suspend fun executeLoadingRatedFilms(sessionId: String): RatedItemsRepository.Result {
        return try {
            val resultMovies =
                service.ratedMovies(accountId = USER.tmdbAccountId, sessionId = sessionId)
    ...
    override suspend fun executeLoadingRatedTvs(sessionId: String): RatedItemsRepository.Result {
        return try {
            val resultTvs = service.ratedTvs(accountId = USER.tmdbAccountId, sessionId = sessionId)
    ...
    override suspend fun executeLoadingRatedEpisodes(sessionId: String): RatedItemsRepository.Result {
        return try {
            val resultEpisodes =
                service.ratedEpisodes(accountId = USER.tmdbAccountId, sessionId = sessionId)
